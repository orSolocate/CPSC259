...
maze_cell** parse_maze( FILE * maze_file, int dimension )
{
    ...
    // INSERT CODE HERE (1 line)
	maze = (maze_cell **)calloc(sizeof(maze_cell*),dimension);

    for ( row = 0; row < dimension; ++row ) {
        // INSERT CODE HERE (1 line)
		maze[row] = (maze_cell*)calloc(sizeof(maze_cell), dimension);
    }
...
void generate_all_paths( maze_cell ** maze, int dimension, int row, int column, char * path )
{
    ...
    if ( // INSERT CODE HERE: Simply return if we hit one of the base cases (there are more than 1)
		...
		column==dimension||row==dimension||column==-1||row==-1|| maze[row][column].visited == VISITED|| maze[row][column].character == MAZE_WALL) {
        return;
	}
...
{
	int i;
	char *shortest=paths[0];
	for (i = 1; i < paths_found; i++)
	{
		if (strlen(paths[i]) < strlen(shortest)) shortest = paths[i];
	}
...
{
	int cost= path_cost(paths[0]);
	char *cheapest = paths[0];
	int i,c;
	for (i = 1; i < paths_found; i++)
	{
		c = path_cost(paths[i]);
