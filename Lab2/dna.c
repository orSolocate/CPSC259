...
void analyze_segments(char * sample_segment, char ** candidate_segments, int number_of_candidates)
{
	...
	for (i = 0; i < number_of_candidates; i++)
	{
		if (strlen(sample_segment) == strlen((*candidate_segments + i)) && !strcmp(sample_segment, *(candidate_segments + i)))
		{
			has_perfect_match = 1;
	...
	else
	{
		...
		for (i = 0; i < number_of_candidates; ++i) {
			...
			//printf(" %d is not a perfect match\n ", i);
		}
	}
	/* End of function */
	return;
...
int calculate_score(char * sample_segment, char * candidate_segment)
{
	...
	int i, j;
	int e1;
	char sam_cur_codon1[4];
	char *pt_sam_cur1;
	char sam_cur_codon2[4];
	char *pt_sam_cur2;

	iterations = (candidate_length - sample_length) / 3 + 1;
	//j = 0;
	for (i = 0; i < iterations; i++)
	{//number of iterations to compare sample to a candidate
		dummy_score = 0;
		///////////////////////////////////////////////////////////////////////////////////////////////////
		for (j = 0; j < sample_length_in_codons; j++) {
			strcpy(sam_cur_codon1, "");
			strcpy(sam_cur_codon2, "");
			pt_sam_cur1 = strncat(sam_cur_codon1, sample_segment + (j * 3), 3);
			pt_sam_cur2 = strncat(sam_cur_codon2, candidate_segment + (j + i) * 3, 3);

			e1 = strncmp(pt_sam_cur1, pt_sam_cur2, 3);
			if (e1 == 0) //equal codons
			{
				dummy_score = dummy_score + 10;
			}
			else if (strcmp(codon_names[get_codon_index(pt_sam_cur1)],
				codon_names[get_codon_index(pt_sam_cur2)]) == 0)
			{
				dummy_score = dummy_score + 5;
			}
			else
			{
				if (pt_sam_cur1[0] == pt_sam_cur2[0])
				{
					dummy_score += 2;
				}
				else dummy_score += is_base_pair(pt_sam_cur1[0], pt_sam_cur2[0]);
				if (pt_sam_cur1[1] == pt_sam_cur2[1])
				{
					dummy_score += 2;
				}
				else dummy_score += is_base_pair(pt_sam_cur1[1], pt_sam_cur2[1]);
				if (pt_sam_cur1[2] == pt_sam_cur2[2])
				{
					dummy_score += 2;
				}
				else dummy_score += is_base_pair(pt_sam_cur1[2], pt_sam_cur2[2]);
			}
		}
